...
class WebSearchAgent:
    ...
    def run_search(query: str, max_results: int = 5) -> List[Dict[str, str]]:
        ...
        if not hasattr(settings, 'google_api_key') or not hasattr(settings, 'google_cse_id') or not settings.google_api_key or not settings.google_cse_id:
            logger.info("Google API credentials not configured, using demonstration mode")
        ...
        try:
            ...
            logger.warning(f"Google search failed: {e}, using demonstration mode")
            return WebSearchAgent._fallback_search(query, max_results)
