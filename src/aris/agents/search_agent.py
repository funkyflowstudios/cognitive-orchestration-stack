...
class WebSearchAgent:
    ...
    def _duckduckgo_search(query: str, max_results: int) -> List[Dict[str, str]]:
        """Search using DuckDuckGo Instant Answer API (completely free, no setup)."""
    ...
    def _bing_search(query: str, max_results: int) -> List[Dict[str, str]]:
        """Search using Bing Web Search API (much easier than Google Cloud)."""
        ...
        params = {
            'q': query,
            'count': min(max_results * 2, 20),  # Get more results for better filtering
            'mkt': 'en-US',
            'safeSearch': 'Moderate'
        }
    ...
    def _google_search(query: str, max_results: int) -> List[Dict[str, str]]:
        ...
        params = {
            ...
            'q': query,
            'num': min(max_results * 2, 10),  # Get more results for better filtering
            'lr': 'lang_en',
            'safe': 'medium',
    ...
    @staticmethod
    def _filter_and_rank_results(results: List[Dict[str, str]], query: str) -> List[Dict[str, str]]:
        ...
        for result in results:
            ...
            if any(char in title + body for char in ['中文', '百度', '经验', '网页', '贴吧', '知道', '音乐', '图片', '视频', '地图', '百科', '文库', '知乎', '问题', '回答']):
                continue
            ...
            irrelevant_keywords = ['login', 'forum', 'register', 'signup', 'tiktok', 'instagram', 'facebook', 'twitter']
    ...
    @staticmethod
    def _calculate_quality_score(result: Dict[str, str], query: str) -> float:
        ...
        title_overlap = len(title_words.intersection(query_words)) / len(query_words) if query_words else 0
        score += title_overlap * 0.4
        
        # Body relevance (30% of score)
        body_words = set(body.split())
        body_overlap = len(body_words.intersection(query_words)) / len(query_words) if query_words else 0
        ...
        authoritative_domains = ['wikipedia.org', 'github.com', 'stackoverflow.com', 'reddit.com', 'medium.com', 'dev.to', 'soundonsound.com', 'musictech.net', 'gearspace.com']
...
class SearchAgent:
    ...
    def search(self, query: str) -> List[Dict[str, Any]]:
        """Perform a web search for the given query using multiple APIs with advanced filtering.
        ...
        try:
            ...
            for result in results:
                ...
                if 'quality_score' not in result:
                    result['quality_score'] = WebSearchAgent._calculate_quality_score(result, query)
            ...
            logger.info(f"Found {len(final_results)} high-quality results for query: {query}")
