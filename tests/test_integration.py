...
class TestFullStackIntegration:
    ...
    async def test_complete_agent_workflow(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            mock_client.generate.side_effect = [
                {"response": '{"plan": ["vector_search", "graph_search"]}'},  # planner
                {
            ...
            assert "AI is a field of computer science" in result.tool_output[0]
            assert "Artificial Intelligence" in result.tool_output[1]
            assert (
    ...
    async def test_agent_workflow_with_async_tools(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            mock_client = MagicMock()
            mock_client.generate.side_effect = [
                {"response": '{"plan": ["vector_search_async", "graph_search_async"]}'},  # planner
                {
            ...
            # Verify results
            assert result.query == "What are async tools?"
            assert len(result.plan) == 2
            assert "vector_search_async" in result.plan
            assert "graph_search_async" in result.plan
            assert len(result.tool_output) == 2
            assert "Async vector search" in result.tool_output[0]
            assert "Async Operations" in result.tool_output[1]
            assert "Async tools provide better performance" in result.response
    ...
    async def test_agent_workflow_error_recovery(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            # Verify that we still get a response despite the error
            assert isinstance(result, AgentState)
            assert result.query == "What is AI?"
            assert len(result.plan) == 2
            # Should have at least one successful tool output
            assert len(result.tool_output) >= 1
            assert "AI is artificial intelligence" in result.tool_output[0]
            assert "Despite some errors" in result.response
    ...
    async def test_agent_workflow_llm_retry_mechanism(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            mock_client.generate.side_effect = [
                ...
                {"response": '{"plan": ["vector_search"]}'},  # Second attempt succeeds
                {
            ...
            assert "vector_search" in result.plan
            assert len(result.tool_output) == 1
            assert "AI encompasses machine learning" in result.tool_output[0]
            assert "Here's a comprehensive answer about AI" in result.response
    ...
    async def test_agent_workflow_fallback_plan(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            assert result.query == "What happens when planning fails?"
            assert len(result.plan) == 1
            assert "vector_search" in result.plan  # Fallback plan
            assert len(result.tool_output) == 1
            assert "Fallback search result" in result.tool_output[0]
            assert "Here's what I found about your query" in result.response
    ...
    def test_agent_workflow_sync_execution(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            # Verify results
            assert isinstance(result, AgentState)
            assert result.query == "How does sync execution work?"
            assert len(result.plan) == 1
            assert "vector_search" in result.plan
            assert len(result.tool_output) == 1
            assert "Synchronous operations" in result.tool_output[0]
            assert "Synchronous execution works perfectly" in result.response
...
class TestAgentStateEvolution:
    ...
    async def test_state_evolution_tracking(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            mock_chromadb = MagicMock()
            mock_chromadb.similarity_search.return_value = ["Vector search result"]
            mock_get_chromadb.return_value = mock_chromadb

            mock_neo4j = MagicMock()
            mock_neo4j.query.return_value = [{"name": "Graph result", "label": "Test"}]
            mock_get_neo4j.return_value = mock_neo4j
            ...
            assert final_state.query == initial_state.query  # Query preserved
            assert len(final_state.plan) == 2  # Plan added by planner
            assert len(final_state.tool_output) == 2  # Tool outputs added by executor
            assert final_state.response != ""  # Response added by synthesizer
            assert final_state.iteration == 1  # Iteration incremented by validator
    ...
    async def test_state_persistence_across_iterations(self, mock_settings):
        ...
        with patch('src.orchestration.nodes._get_ollama_client') as mock_get_client, \
            ...
            assert result.iteration == 2  # 1 + 1
            assert result.query == "Test query"  # Query preserved
            assert len(result.plan) == 1  # Plan added
            assert len(result.tool_output) == 1  # Tool output added
            assert result.response != ""  # Response added